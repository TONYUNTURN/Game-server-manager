...
一个基于 Shell 的轻量级 Linux 游戏服务器管理器，为 NAT VPS 和独立服务器设计。
...
- **性能优化**：
  - **CPU Affinity**：自动绑定核心，避免通过 CPU 0 调度，提升性能。
...
### 菜单选项说明
1.  **[1] 安装/更新服务器**
    - 输入 Steam AppID（例如 `380870` 是僵尸毁灭工程）。或游戏名（如 `Project Zomboid`）。
    - 脚本会自动调用 SteamCMD 下载或验证服务端文件。
    - 安装完成后会自动记录到已知服务器列表（`known_servers.json`）。
